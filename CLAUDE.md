...
- Threat model gap: creation-time passphrases (`keys generate`/`encrypt`/`add`) still accept `RWA_PASSPHRASE` headless — an injected agent could re-encrypt a plaintext wallet with an unknown passphrase (lockout, not exfiltration); TTY-gating creation is a possible future hardening
