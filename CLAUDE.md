...
- When `route_unfillable` is terminal (the RFQ MM had no inventory and there is no AMM fallback — thin single-name GM tokens like PFEon/LLYon are minted just-in-time only on Ondo's primary market, which Jupiter can't reach), the human/JSON message names the symbol and points to app.ondo.finance instead of dumping the per-backend quote wall; `error_kind` stays `route_unfillable`
