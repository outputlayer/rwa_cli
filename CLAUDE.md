...
- For full exit: `close-all -> reclaim -> send USDC all -> send SOL all` (to an ALLOWED address — if the wallet has a non-empty send policy, `send` to anywhere else fails `recipient_not_allowed`)
