...
- When `route_unfillable` is terminal, the human/JSON message names the symbol and states the cause instead of dumping the per-backend quote wall; `error_kind` stays `route_unfillable`. NOTE: for thin RFQ tokens (PFEon/LLYon) this is frequently a FALSE negative — on-chain evidence shows the market maker funds the fill just-in-time at `/execute` (mints in the same block), which the local pre-sign simulation, run before submission, can't see; so the CLI refuses to sign a swap that would actually land. The message says so honestly; the behavioural fix (trusting the JIT fill for managed routes) is tracked separately
