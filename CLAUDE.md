...
- Exit codes: 75 (EX_TEMPFAIL) for transient failures worth retrying (`rpc_unavailable`, `execute_unavailable`, `confirmation_timeout`, lock contention, plus the transient `/execute` kinds above: `failed_to_land`, `rfq_failed_to_land`, `internal_error`, `quote_expired`, `missing_cached_order`, `swap_rejected`, `route_unfillable`); 1 for everything else
