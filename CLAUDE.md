...
- `--max-bps <N>` (buy/sell/baskets/close-all) rejects trades whose quoted all-in cost (spread + fee) exceeds N bps; `RWA_MAX_BPS` is the env default; surfaced as `cost_too_high` (per-item `failed[]` entries in multi-trade commands). All-in cost nets the fee against price-impact (`fee_bps − slippage_pct·100`), so a **favorable spread offsets the fee** — a quote with a 10-bps fee but a better-than-quoted price can pass `--max-bps 5` (even `--max-bps 0`); the cap is on net cost, not the raw fee. A quote reporting neither slippage nor fee data (cost unmeasurable) fails **closed** under `--max-bps` — see the slippage-measurability note below
...
- `--equal` (buy-basket, requires `--total`) splits the total EVENLY across bare symbols (no per-token amounts): `buy-basket --total 1000 --equal TSLA NVDA SPY` → ~333.33 each (floor + dust to first, same 5 USDC/item minimum). It's the only clean way to equal-weight an arbitrary N under the exact-100 `--total` rule (3×33.33 ≠ 100). `--from-file <path>` reads tokens from a file instead of args (`-` = stdin; whitespace/newline separated, `#` comment and blank lines ignored); mutually exclusive with positional tokens. Both compose with all three amount modes
...
- `buy-basket`/`sell-basket`/`close-all` (real path, not `--dry-run`) derive their JSON `status` from item outcomes: `"success"` (every item landed, or nothing to do — e.g. an empty `close-all`), `"partial"` (a mix; inspect `failed[]`), or `"error"` (every item landed in `failed[]`, nothing moved; exit non-zero). `skipped[]` (close-all's non-sellable positions) never drives `"error"`, but a skip that a repeat run WOULD clear — `trading_paused`, `not tradable in current session`, `market data unavailable` — reports `"partial"`, carries `incomplete_reason`, and exits **75**: the portfolio is not empty, so a shell chain (`close-all && reclaim && send`) must stop instead of withdrawing the gas needed to sell the rest. Dust below the $1.50 minimum is `retryable: false` and keeps `"success"` with exit 0 — no retry can sell it (a `"partial"` driven only by `failed[]`, on baskets or on close-all, still exits 0 — unchanged). `--dry-run` always exits 0 (still carries `incomplete_reason` so a preview warns about what won't sell). The all-failed case still emits the full envelope (`bought`/`sold`/`failed`/`skipped` — not the generic top-level `error`/`error_kind` shape); only the exit code changes
- `portfolio` uses nested JSON: `cash.*` plus `gm_positions.*`; every position carries `sector`/`asset_class`/`region`/`type`/`tags` (Ondo catalog labels, omitted when unknown) so scripts never need a join against `gm search`
- `--view <term>…` (portfolio only) slices the portfolio with ONE polymorphic flag: a reserved category name (`sector`, `region`, `class`, `type`, `factor`) SPLITS the output into per-group blocks; an Ondo tag label (`Dividend`, `Healthcare`, `ETF`, …) or a ticker (`MRNA`) FILTERS it — reserved category names always win over a same-spelled catalog label. Repeatable and comma-separated; `tag:`/`token:` prefixes, or any category name as a prefix (`sector:`/`region:`/`class:`/`type:`/`factor:`), skip auto-detection. A `<category>:<label>` whose label belongs to a DIFFERENT category names that category in the error rather than running did-you-mean against itself. At most one category per invocation. Labels of the same category OR together, different categories AND. `gm_alloc_pct` and `GM TOTAL` are ALWAYS relative to the full portfolio — a filter never rewrites them; a partial filter adds a `MATCHED` line (filter-only mode; with a split, coverage is shown by the `view: ... · N positions · Y% of GM` header line instead — no `MATCHED` line is printed when a split is active, even for a partial filter). With `--view`, JSON's `gm_positions.positions[]` is likewise narrowed to only the matched positions while `gm_positions.value_usd` stays full-portfolio — use `view.matched_value_usd` for the slice's total. Factor tags are multi-valued (333 of 444 live assets carry 2+), so `--view factor` produces OVERLAPPING groups that do NOT sum to the total — JSON marks this as `overlapping: true` and human output prints a note. Positions without a label in the split category land in `Unclassified` (literal, no invented names). JSON adds `gm_positions.view` and `gm_positions.groups` ONLY when the flag is given; `groups` is always present then, with `group: null` when there is no split, so agents parse one shape. A term matching nothing is exit 0 with `groups: []`; an unrecognized term is `invalid_view` (exit 1); ALL terms blank (e.g. `--view ""`, reachable via an unset `--view "$VAR"`) is also `invalid_view` (exit 1) — deliberate fail-closed, since silently returning the whole portfolio in slice format would mislead
- Every CLI money operation is appended to a per-wallet trade ledger (`~/.config/rwa/ledger/<pubkey>.jsonl`, raw units). Each entry carries a `prev` hash chain link (hex of the first 16 bytes of SHA-256 over the raw previous line; genesis sentinel is the hash of empty bytes) — tamper-evident, not tamper-proof. `verify_chain_at` also requires the file's first line's `prev` to equal the genesis sentinel, so deleting a PREFIX of the file (genesis + earliest entries) is caught as `BrokenAt(1)` rather than reading as `Ok`. Known limit: the chain only validates each entry against its predecessor, so the file's NEWEST entry has no successor to check it against — editing the last line's own fields in place is undetectable (a HEAD checkpoint was evaluated and REJECTED 2026-07: the chain holds no secret, so anyone who can edit the file can re-hash every subsequent link anyway — the chain is anti-accident, not anti-attacker; a checkpoint only helps if anchored outside the file, disproportionate for an informational `pnl`. Revisit only if the ledger ever feeds tax/reporting). `gm pnl` builds average entry price, realized and unrealized P&L **from buys/sells only** (deposits and inbound transfers are ignored by design — the CLI can't see or price them), with ONE transfer exception: a CLI-recorded `send_out` of a GM token reduces the position at average cost with no realized impact (the CLI recorded that transfer itself; keeping the qty showed phantom holdings with "unrealized" P&L on tokens the wallet no longer holds — fixed in 0.7.13); sells beyond CLI-recorded buys are flagged `oversold` and excluded rather than mispriced. `pnl` discloses its scope: human mode ends with a note that only CLI trades are counted plus the ledger file path (delete the file to reset history); JSON carries the same path as optional `ledger_path`. `pnl --all` compares every wallet that ever traded through the CLI — one row per ledger file (pure ledger-dir enumeration, works headless, needs NO key material, so removed/unregistered wallets still appear), sorted by total P&L descending (unknowable totals last); JSON shape `{wallets:[{wallet,trades_recorded,invested_usdc,market_value_usdc?,unrealized_usdc?,realized_usdc,total_pnl_usdc?,ledger_integrity}]}`. `PnlJson.ledger_integrity` is ALWAYS present: `"ok"` (chain intact), `"legacy"` (pre-chain entries present, unbroken), or `"broken@line N"` (first line whose `prev` doesn't match); a broken chain warns on stderr (human mode only; in `--json` the signal is the `ledger_integrity` field itself) but never fails `pnl`
...
rwa gm buy-basket --total 1000 --equal TSLA NVDA SPY -y   # even split across N
rwa gm buy-basket --from-file basket.txt --total 500 -y   # tokens from a file
rwa --json gm search --tradable-only --sector Technology | jq -r '.items[].symbol' | xargs rwa gm buy-basket --total 1000 --equal -y   # filter-driven bulk (compose)
...
rwa gm pnl --all                    # compare every wallet that ever traded via the CLI (per-ledger rows, no keys needed)
...
rwa keys policy show                              # list the send allowed-recipient list (operational passphrase chain)
rwa keys policy allow <ADDR> --label cold         # add an address to the allowed list (admin: typed passphrase, TTY-only)
rwa keys policy remove <ADDR>                     # remove an address from the allowed list (admin: typed passphrase, TTY-only)
...
- Swaps with >3% slippage are blocked after all retries exhausted. Slippage is derived from the quote's price-impact fraction (converted to a percent) or, when absent, the in/out USD value delta; when a quote reports **neither** (unmeasurable), the 3% block is not applied — it fails open (an honest thin-token quote may legitimately omit both) but warns on stderr. `--max-bps` is stricter: an explicit ceiling means cost must be verified, so an unmeasurable quote fails **closed** with `cost_too_high` instead of silently passing
...
- `TransactionErrorKind` (Solana transaction layer, feeds `error_kind` via `classify_error` like the kinds above): `missing_blockhash` / `invalid_blockhash` — `getLatestBlockhash` returned no blockhash or an undecodable one; **transient** (exit 75), a retry fetches a fresh blockhash. `simulation_failure` — pre-send simulation proved the transaction would fail on-chain, so nothing was submitted; **permanent** (exit 1), do not retry as-is
- Jupiter `/execute` failure kinds (`ExecuteFailureKind`, mapped from Jupiter's numeric error codes): `failed_to_land`, `rfq_failed_to_land`, `internal_error`, `quote_expired`, `missing_cached_order`, `swap_rejected`, and `route_unfillable` are **transient (exit 75)** — each carries a retry action (`RefreshOrder`/`RetrySameOrder`) and the CLI's own retry loop already resubmits/refetches these before giving up, so a fresh command invocation may still succeed (same treatment as `execute_unavailable`, already exit 75). **Changed in 0.7.9:** `swap_rejected`, `missing_cached_order`, and `route_unfillable` now exit 75 (were exit 1) — their `retry_action` is `RefreshOrder`, so classifying them permanent was a false abort signal. `invalid_signed_transaction`, `invalid_message_bytes`, `unknown_aggregator_error`, `unknown_rfq_error`, `invalid_payload`, and `unknown` carry no retry action and stay **permanent (exit 1)** — retrying as-is won't help. `is_transient_kind` derives this split straight from `ExecuteFailureKind::retry_action()`, not a hand-maintained list
- Input/environment error kinds (previously `error_kind: null`): `unknown_token` (symbol not in the GM list), `invalid_amount` (bad format/too many decimals/bad percentage), `invalid_address` (send recipient), `unknown_wallet` (`--wallet`/`RWA_WALLET` miss; message lists available names), `self_send` (send to own address), `reveal_required` (`keys export --json` without `--reveal`), `interactive_required` (admin command — keys export --reveal / keys decrypt — run without a TTY: type the passphrase interactively; env/keychain are not consulted for these), `recipient_not_allowed` (send recipient not in the wallet policy's allowed list; a human adds addresses with `keys policy allow <ADDR>` — agents must not attempt workarounds), `lock_contention` (another rwa process holds the lock; **transient — exit 75 in both human and json modes**), and `invalid_view` (`gm portfolio --view` term matched no category, ticker, or Ondo tag label; exit 1, not transient)
...
- Known limit (audit L5, accepted): the pre-sign input-debit is `input_pre − input_post` where `input_pre` (getMultipleAccounts) and `input_post` (simulateTransaction) come from two separate RPC reads that can land on different slots. A concurrent debit to the same input ATA between the reads (e.g. two parallel `buy-basket` items sharing the USDC ATA within one process) could under-report the debit and let an over-spending CPI/RFQ route pass. Not attacker-triggerable (needs a sub-second same-ATA race the caller can't force), and the on-chain `otherAmountThreshold` min-output still bounds the fill; a slot-equality guard was evaluated and rejected because default multi-provider RPC + slower `simulateTransaction` make honest slot drift common (it would refuse legitimate swaps). A correct fix (re-read the input ATA post-sim, refuse on change) costs an RPC round-trip per swap and is deferred rather than paid on every trade for this low-severity race
...
- **Send policy (allowed recipients)**: `keys policy show/allow/remove` manages an opt-in per-wallet whitelist for `send`, stored INSIDE the age-encrypted wallet payload (payload v3: `{key, mnemonic, policy: {allowed_recipients: [{address, label, added_at}]}}`) — never written outside the encrypted container. `show` is operational-class (env → keychain → TTY, same chain as trading); `allow`/`remove` are admin-class (typed passphrase at a live TTY only — `RWA_PASSPHRASE`/keychain deliberately not consulted, headless → `interactive_required`), since they mutate where money can go. Enforcement lives in `send`'s pre-network gate (fires before any RPC/Jupiter call, `--dry-run` included): **empty/absent policy = behavior unchanged** (no gate at all); once the list is non-empty, a **non-interactive** caller (`-y` OR `--json` OR no TTY — any one of the three) sending to an address not on the list is refused outright with `error_kind: recipient_not_allowed`, with NO prompt — this is safe by design, not a bug (an agent driving `-y` can never talk its way past the gate). A **live human at an interactive TTY** instead gets a two-step escape hatch: type the LAST 6 characters of the address (anti-clipboard-swap — anything else, including a blank line, cancels), THEN type the wallet passphrase, which is re-verified by actually re-decrypting the wallet file (mirrors `store_passphrase`'s verify-before-trust) — a PTY-driving agent that only knows the address it typed cannot pass this second step; wrong passphrase cancels with no network access ("Passphrase rejected — send cancelled"). Same caveat as the admin-gate/keychain-ACL degradation above: this second factor is human-only only when the wallet has no `RWA_PASSPHRASE` set in the environment — if it is set, a co-resident PTY-driving agent can read it and type it back, collapsing the escape hatch to the same env-leak level as `RWA_PASSPHRASE` itself (the recommended keychain-only, no-env-passphrase setup avoids this). This path exists only for encrypted wallets (plaintext carries no policy, so there's nothing to gate). A listed address, or self-send, is never gated. **Plaintext wallets can't carry a policy** (`policy show` reports `policy: null`; `policy allow/remove` error "Send policy requires an encrypted wallet. Encrypt first: `rwa keys encrypt`") — the whitelist only exists inside the age container. Since an empty list is opt-in-inactive, `keys policy remove` on the LAST allowed address silently re-disables the gate — `policy_edit` warns rather than blocks (draining it on purpose, e.g. rotating out a retired cold-storage address, is legitimate): on the remove that reaches zero, `--json` adds `policy_now_empty: true` and human mode prints a stderr warning ("Allowed-recipient list is now EMPTY — the send gate is disabled; non-interactive sends to any address will proceed. Re-add an address to re-enable it."); a remove that leaves entries behind, or any `allow`, never sets the flag.

## Agent usage rules

- Always prefer `rwa --json`
- Use `-y` only for real execution
- **`--json` without `-y` never executes** (breaking since v0.6.0): the six gated money-moving commands (buy, sell, send, buy-basket, sell-basket, close-all) run with `--json` alone fail closed instead of running non-interactively. An otherwise-valid invocation returns `confirmation_required` (exit 1); a precondition failure surfaces its own kind first (e.g. `insufficient_funds`, `no_position`, `amount_below_minimum`) — the guarantee is "never executes", not "always `confirmation_required`". `close-all` on an empty wallet returns `status:success` (nothing to sell) before the gate, and returns `status:"partial"` with `incomplete_reason` and exit 75 when every position was skipped as temporarily unsellable — both bypass the gate because there is nothing to sell. `reclaim` reclaims rent to your own wallet and runs without confirmation. This also gates auto-gas refuel: json auto-approves the refuel only when `-y` is also passed. Add `-y` to execute, or `--dry-run` to preview
...
- Exit codes: 75 (EX_TEMPFAIL) for transient failures worth retrying (`rpc_unavailable`, `execute_unavailable`, `confirmation_timeout`, lock contention, `missing_blockhash`, `invalid_blockhash`, `close-all`'s own `"partial"` when a retryable `skipped[]` entry remains, plus the transient `/execute` kinds above: `failed_to_land`, `rfq_failed_to_land`, `internal_error`, `quote_expired`, `missing_cached_order`, `swap_rejected`, `route_unfillable`); 1 for everything else
...
- For full exit: `close-all -> reclaim -> send USDC all -> send SOL all` (to an ALLOWED address — if the wallet has a non-empty send policy, `send` to anywhere else fails `recipient_not_allowed`); stop the chain if `close-all` exits 75 — that means a retryable skip left sellable positions in the wallet, and reclaiming/sending onward would withdraw the gas needed to sell them later
...
- On `error_kind: recipient_not_allowed`, STOP and tell the human — `rwa keys policy allow <ADDR>` is admin-class (typed passphrase, TTY-only) and cannot be run by an agent; never retry, never try a different amount/wording, never suggest the human disable the policy as a workaround
