...
- `buy-basket`/`sell-basket`/`close-all` (real path, not `--dry-run`) derive their JSON `status` from item outcomes: `"success"` (every item landed, or nothing to do — e.g. an empty `close-all`), `"partial"` (a mix; exit 0, inspect `failed[]`), or `"error"` (every item landed in `failed[]`, nothing moved; exit non-zero). `skipped[]` (close-all's tiny/non-tradable positions) is NOT a failure and never drives `"error"`. The all-failed case still emits the full envelope (`bought`/`sold`/`failed`/`skipped` — not the generic top-level `error`/`error_kind` shape); only the exit code changes
...
- Exit codes: 75 (EX_TEMPFAIL) for transient failures worth retrying (`rpc_unavailable`, `execute_unavailable`, `confirmation_timeout`, lock contention, `missing_blockhash`, `invalid_blockhash`, plus the transient `/execute` kinds above: `failed_to_land`, `rfq_failed_to_land`, `internal_error`, `quote_expired`, `missing_cached_order`, `swap_rejected`, `route_unfillable`); 1 for everything else
