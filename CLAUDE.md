...
- Parallel quotes are **staggered** (`RWA_QUOTE_STAGGER_MS`, default 350ms between launches): firing a basket's quotes all at once bursts Jupiter's per-wallet rate limit, whose 429 → retry backoff (0.8/1.6/3.2s) costs far more than the stagger. The stagger keeps the overlap while dodging the burst; best case a 3-token basket quotes in ~1.3s. Without an API key Jupiter's per-wallet limit still bounds parallel throughput — set `RWA_JUPITER_API_KEY` for the real parallel unlock (then lower the stagger toward 0)
