...
- Trade/send/close-all JSON shapes are covered by regression tests and kept stable. Surfaced `error_kind` values include `market_closed`, `not_tradable`, `slippage_too_high`, `cost_too_high`, `confirmation_timeout`, `on_chain_failure`, `execute_unavailable`, `route_unfillable`, `rpc_unavailable`, `amount_below_minimum`.
