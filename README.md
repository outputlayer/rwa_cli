...
- Jupiter `/execute` failure kinds (`ExecuteFailureKind`, mapped from Jupiter's numeric error codes): `failed_to_land`, `rfq_failed_to_land`, `quote_expired`, `swap_rejected`, `internal_error`, `missing_cached_order` are **transient (exit 75)** — same contract as `execute_unavailable`/`route_unfillable` above, since each already gets an internal retry before surfacing. `invalid_signed_transaction`, `invalid_message_bytes`, `unknown_aggregator_error`, `unknown_rfq_error`, `invalid_payload`, `unknown` are **permanent (exit 1)**.
