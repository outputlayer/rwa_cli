...
- **`--view` slices `portfolio`.** A category name (`sector`, `region`, `class`, `type`, `factor`) splits the output into blocks; a tag label (`Dividend`, `Healthcare`, …) or ticker (`MRNA`) filters it instead — comma-separated and repeatable, same-category terms OR together, different categories AND (at most one category per call). `gm_alloc_pct`/`GM TOTAL` always reflect the whole portfolio, never the filtered slice; JSON's `gm_positions.positions[]` IS narrowed to the matched subset, though — use `view.matched_value_usd` for the slice's total. `--view factor` groups overlap (tokens carry 2+ factor tags) and won't sum to the total — JSON marks it `overlapping: true`. An unrecognized term, or all terms blank, fails closed with `invalid_view` (exit 1) rather than silently showing everything. Every position also carries `sector`/`asset_class`/`region`/`type`/`tags`, `--view` or not.
...
- **Multi-token commands run in parallel** by default (internally bounded); `--sequential` is the rate-limit fallback. `close-all` is the canonical exit — it skips dust and paused/off-session positions and reports what it skipped; a skip that a retry would clear makes the run `"partial"` and exit **75** instead of a plain success.
...
Every command supports `--json` — a stable contract with typed `error_kind` values and exit code 75 for retry-worthy transient failures (1 otherwise). Rules: prefer `--json`, use `-y` only for real execution, never run wallet-changing commands as parallel shell processes (multi-token commands parallelize internally). **`--json` without `-y` never executes** the six gated money-moving commands (buy, sell, send, buy-basket, sell-basket, close-all) — it fails closed instead of running non-interactively (an otherwise-valid trade returns `error_kind: confirmation_required` (exit 1); a precondition failure like `insufficient_funds`/`no_position` surfaces its own kind first); add `-y` to execute or `--dry-run` to preview. `reclaim` reclaims rent to your own wallet and runs without confirmation. Baskets and `close-all` add a third envelope `status`, `"partial"` — some items landed, some are in `failed[]` — alongside `"success"` (all landed, or nothing to do) and the all-failed `"error"` (exit non-zero); always read per-item `failed[]` for the reason, even on `"partial"`. A `failed[]`-only `"partial"` exits 0, but for `close-all` a `"partial"` caused by a retryable `skipped[]` entry (`trading_paused`, `not tradable in current session`, `market data unavailable`) also carries `incomplete_reason` and exits **75** — the portfolio isn't empty, so a `close-all && reclaim && send` chain must stop there; dust (`below $1.50 minimum`) never does this and keeps `"success"`/exit 0. Manual skill install: [outputlayer/rwa_skills](https://github.com/outputlayer/rwa_skills).
...
- **Optional send allowlist**: `keys policy allow <ADDR>` (encrypted wallets only, typed passphrase) locks `send` to a whitelist stored inside the encrypted wallet — once set, a non-interactive send to an unlisted address fails closed with `recipient_not_allowed` instead of moving funds.
- `portfolio` JSON: `cash.{sol,usdc}` + `gm_positions.{value_usd, change_24h_usd, positions[]}`. Surfaced `error_kind` values: `market_closed`, `not_tradable`, `slippage_too_high`, `cost_too_high`, `condition_not_met`, `trading_paused`, `route_unfillable`, `rpc_unavailable`, `insufficient_funds`, `amount_below_minimum`, `no_position`, `confirmation_timeout`, `on_chain_failure`, `execute_unavailable`, `confirmation_required` (`--json` without `-y`), plus input kinds `unknown_token`, `invalid_amount`, `invalid_address`, `unknown_wallet`, `self_send`, `reveal_required`, `interactive_required`, `recipient_not_allowed`, `lock_contention` (transient, exit 75), and `invalid_view` (`gm portfolio --view` term matched no category, ticker, or Ondo tag label; exit 1, not transient). Transaction-layer kinds (`TransactionErrorKind`): `missing_blockhash` / `invalid_blockhash` (RPC blockhash fetch failed or was undecodable — transient, exit 75, retry gets a fresh blockhash) and `simulation_failure` (pre-send simulation proved the tx would fail on-chain, nothing was submitted — permanent, exit 1).
- Jupiter `/execute` failure kinds (`ExecuteFailureKind`, mapped from Jupiter's numeric error codes): `failed_to_land`, `rfq_failed_to_land`, `quote_expired`, `swap_rejected`, `internal_error`, `missing_cached_order`, and `route_unfillable` are **transient (exit 75)** — each carries a `RefreshOrder`/`RetrySameOrder` retry action and gets an internal retry before surfacing, so a fresh command invocation may still succeed (same treatment as `execute_unavailable`, already exit 75). New in 0.7.9: `swap_rejected`, `missing_cached_order`, and `route_unfillable` become exit 75 here (were exit 1) — their `retry_action` is `RefreshOrder`. `invalid_signed_transaction`, `invalid_message_bytes`, `unknown_aggregator_error`, `unknown_rfq_error`, `invalid_payload`, `unknown` are **permanent (exit 1)**.
